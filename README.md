...
| **スニペット再読込 / 設定を開く** | 定型文設定 `snippets.txt` の再読み込みと編集（下記「スニペット」参照） |
...
`%USERPROFILE%\.myahk_v2\snippets.txt` にトリガーと本文を書くと、トリガーを打った直後に本文へ置き換わります（espanso / Lintalist の代替）。
...
- `[トリガー] ラベル` が見出し、次の見出しまでが本文（複数行可）
- 同じトリガーを複数書くとカーソル位置に候補メニューが出る
- `{date:書式}`・`{clipboard}`・`{input:見出し}`（入力ダイアログ）・`$|$`（貼り付け後のカーソル位置）が使える
